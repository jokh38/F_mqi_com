...
class ErrorClassifier:
    ...
    SUBPROCESS_RETURN_CODE_CATEGORIES = {
        ...
        range(126, 128): ErrorCategory.SYSTEM,   # Permission denied, command not executable
    ...
    MESSAGE_PATTERNS = [
        # Network patterns
        (re.compile(r'connection.*tim[ed].*out|timeout.*connection|connection.*timed.*out', re.IGNORECASE), ErrorCategory.NETWORK),
        (re.compile(r'connection.*refused|refused.*connection', re.IGNORECASE), ErrorCategory.NETWORK),
        (re.compile(r'connection.*reset|reset.*connection', re.IGNORECASE), ErrorCategory.NETWORK),
        (re.compile(r'network.*unreachable|unreachable.*network', re.IGNORECASE), ErrorCategory.NETWORK),
        (re.compile(r'host.*unreachable|unreachable.*host', re.IGNORECASE), ErrorCategory.NETWORK),
        (re.compile(r'name.*resolution.*failed|dns.*failed', re.IGNORECASE), ErrorCategory.NETWORK),
        
        # System patterns
        (re.compile(r'permission.*denied|access.*denied', re.IGNORECASE), ErrorCategory.SYSTEM),
        (re.compile(r'file.*not.*found|no.*such.*file', re.IGNORECASE), ErrorCategory.SYSTEM),
        (re.compile(r'disk.*full|no.*space.*left', re.IGNORECASE), ErrorCategory.SYSTEM),
        (re.compile(r'out.*of.*memory|memory.*exhausted', re.IGNORECASE), ErrorCategory.SYSTEM),
        
        # Configuration patterns
        (re.compile(r'invalid.*config|config.*error|configuration.*error', re.IGNORECASE), ErrorCategory.CONFIGURATION),
        (re.compile(r'missing.*required|required.*missing', re.IGNORECASE), ErrorCategory.CONFIGURATION),
        (re.compile(r'invalid.*format|format.*invalid', re.IGNORECASE), ErrorCategory.CONFIGURATION),
    ]
    ...
    def _classify_subprocess_error(self, error: subprocess.CalledProcessError) -> ErrorCategory:
        """
