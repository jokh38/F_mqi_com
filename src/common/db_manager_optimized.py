...
class QueryPerformanceMetrics:
    ...
    def add_query(self, execution_time: float, was_cached: bool = False, slow_threshold: float = 0.05) -> None:
...
class OptimizedDatabaseManager:
    ...
    def __init__(
        ...
        Args:
            ...
            enable_wal_mode: Whether to use WAL mode for better concurrency
    ...
    def get_cases_by_status(self, status: str, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        cache_key = f"cases_by_status_{status}_{limit}" if self.enable_cache and limit and limit <= 50 else None
    ...
    def get_cases_by_priority_and_status(self, status: str, min_priority: int = 1, limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_execution_time_ms": round(self.metrics.average_execution_time * 1000, 2),
            "total_execution_time_ms": round(self.metrics.total_execution_time * 1000, 2),
    ...
    def get_gpu_resource_by_case_id(self, case_id: int) -> Optional[Dict[str, Any]]:
        ...
        results = self._execute_with_metrics(
            "SELECT * FROM gpu_resources WHERE assigned_case_id = ?",
            (case_id,)
        )
        return dict(results[0]) if results else None
    ...
    def get_resources_by_status(self, status: str) -> List[Dict[str, Any]]:
        ...
        results = self._execute_with_metrics(
            "SELECT * FROM gpu_resources WHERE status = ?",
            (status,)
        )
        return [dict(row) for row in results]

    def update_gpu_status(self, pueue_group: str, status: str, case_id: Optional[int] = None) -> None:
