...
class TestConfigManager:
    ...
    def test_validate_config_with_missing_required_section_fails(self):
        ...
        try:
            with pytest.raises(ConfigValidationError, match="Missing required section: hpc"):
    ...
    def test_validate_config_with_missing_required_field_fails(self):
        ...
        config_path = self.create_temp_config_file(invalid_config)
        try:
            with pytest.raises(ConfigValidationError, match="Missing required field: hpc.host"):
                ConfigManager(config_path)
        finally:
            os.unlink(config_path)
    ...
    def test_validate_config_with_invalid_type_fails(self):
        ...
        config_path = self.create_temp_config_file(invalid_config)
        try:
            with pytest.raises(ConfigValidationError, match="Invalid type for scanner.quiescence_period_seconds"):
                ConfigManager(config_path)
        finally:
            os.unlink(config_path)
    ...
    def test_get_without_default_raises_error_when_missing(self):
        ...
        try:
            config_manager = ConfigManager(config_path)
            with pytest.raises(ConfigValidationError, match="Configuration key not found: nonexistent.key"):
    ...
    def test_get_section_with_missing_section_raises_error(self):
        ...
        try:
            config_manager = ConfigManager(config_path)
            with pytest.raises(ConfigValidationError, match="Configuration section not found: nonexistent"):
